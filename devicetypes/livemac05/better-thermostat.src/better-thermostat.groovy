...
metadata {
	...
	definition (name: "Better Thermostat", namespace: "livemac05", author: "livemac05@gmail.com") {
	...
	tiles {
        ...
        */
        ...
        valueTile("noGoLabel", "device.configure", decoration: "flat", width: 3, height: 1) {
        	state "configure", label: 'Use Smarter Thermostat Instead'
        }
        valueTile("moreLabel", "device.configure", decoration: "flat", width: 3, height: 1) {
        	state "configure", label: '(In Hallway Room, and Things List)'
        }
        
        main "temperature"
        details(["noGoLabel", "moreLabel", "temperature", "humidity", "battery", "refresh"])
        
        
    }
}

/*def setSensorTemperature (value) {
	if (value) {
    	log.debug "Sending temp event to ${value}"
    	//sendEvent(name: 'temperature', value: value, unit: temperatureUnit, state: "cool")
    	//sendEvent(name: 'temperature', value: value, unit: temperatureUnit, state: "heat")
    	sendEvent(name: 'temperature', value: value, unit: "fahrenheit", state: "auto")
    }
}
